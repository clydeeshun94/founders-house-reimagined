...
/* Width grows independently of height now — cards get wider and squarer
   without stretching the imagery. */
const CARD_W = "min(96vw, 720px)";
const CARD_H = "373px"; /* held at the original 560px-wide, 3:2 height */
...
function Card({ item }: { item: (typeof GALLERY_ITEMS)[number] }) {
  return (
    <div style={{ width: CARD_W, height: CARD_H, borderRadius: 0, overflow: "hidden", position: "relative", boxShadow: "0 16px 48px rgba(0,0,0,0.12)", flexShrink: 0 }}>
      ...
      {/* faint centered scrim so red text stays legible over busy imagery, without flattening the whole photo */}
...
export function Gallery() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      /* sine x-offset, now scroll-linked instead of static so the stagger
         itself has motion, not just a fixed layout offset */
